...
namespace Entertainment.Persistance.Concretes.Repositories.BookNoteRepositories
{
    internal class BookNoteReadRepository : IBookNoteReadRepository
    {
        ...
        public BookNoteReadRepository(IDapperBaseReadService service)
        {
            _service = service;
        }
        ...
        {
            var bookNoteList = _service.GetAll<BookNote>("SELECT * FROM \"BookNotes\"", new { });
            return bookNoteList;
        }

        public async Task<List<BookNote>> GetAllAsync()
        {
            var bookNoteList = await _service.GetAllAsync<BookNote>("SELECT * FROM \"BookNotes\"", new { });
            return bookNoteList;
        }

        public BookNote GetById(string id)
        {
            var bookNote = _service.Get<BookNote>("SELECT * FROM \"BookNotes\" WHERE Id = @id", new { id });
            return bookNote;
        }

        public async Task<BookNote> GetByIdAsync(string id)
        {
            var bookNote = await _service.GetAsync<BookNote>("SELECT * FROM \"BookNotes\" WHERE Id = @id", new { id });
            return bookNote;
        }

        public List<BookNote> GetUsersAll(string userId)
        {
            var bookNoteList = _service.GetAll<BookNote>("SELECT * FROM \"BookNotes\" WHERE UserId = @userId", new { userId });
            return bookNoteList;
        }

        public async Task<List<BookNote>> GetUsersAllAsync(string userId)
        {
            var bookNoteList = await _service.GetAllAsync<BookNote>("SELECT * FROM \"BookNotes\" WHERE UserId = @userId", new { userId });
            return bookNoteList;
        }
    }
}
